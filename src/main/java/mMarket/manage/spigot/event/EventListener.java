...
public class EventListener implements Listener {
    ...
    public void SwapHandDisable(PlayerSwapHandItemsEvent e) {
        Player player = e.getPlayer();
        if (plugin.getConfig().getInt(category_player + "swap-hand") == 1 && !player.hasPermission(plugin.getConfig().getString("permissions.swap-hand"))) {
            e.setCancelled(true);
    ...
    public void BlockContains(AsyncPlayerChatEvent e) { // АСУНС СКАМ - ОКУП ЭВЕНт
        ...
        if (player.isOp()) {
            ...
            for(String s : plugin.getConfig().getStringList("block-contains-chat")) { // где фореачи нищие разрабы???
    ...
    public void JoinInfo(PlayerJoinEvent e) {
        ...
        if (plugin.getConfig().getInt(category_chat + "join-info-msg") == 1) {
            var2.sendMessage(plugin.getConfig().getString("messages.join-info").replace('&', '§'));
        }
